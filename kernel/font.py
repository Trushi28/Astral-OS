#!/usr/bin/env python3
"""
Generate a basic 8x16 bitmap font for Astral OS
Output: font.bin (95 characters * 16 bytes = 1520 bytes)
Characters: ASCII 32 (space) through 126 (~)
"""

import struct

# 8x16 font data - each character is 16 bytes (one byte per row)
...
if __name__ == "__main__":
    ...
    with open("kernel/font.bin", "wb") as f:
        f.write(font_data)
    print(f"Generated font.bin: {len(font_data)} bytes ({len(font_data)//16} characters)")